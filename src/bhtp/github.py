...
class Github:
    ...
    def load_ohlcv_for_month(self, year:int=2023, month:int=4, verbose=False):
        """
        Load and aggregate OHLCV data from multiple CSV files for a given month.

        This method retrieves OHLCV (Open, High, Low, Close, Volume) data for a specified month and year from the github repository.
        The CSV files in the GitHub repository are filtered by month and concatenated into a single DataFrame.
        
        Parameters:
        ----------
        year : int, optional
            The year for which to retrieve the data. Must be 2023 or higher. Default is 2023.
        month : int, optional
            The month for which to retrieve the data (1-12). Default is April (4).
        ...
        ------
        ValueError
            If the `year` is less than 2023 or the `month` is outside the range 1-12.
        
        Example:
        --------
        >>> github = Github(owner="MapleFrogStudio", repository="DATA-2023-04", branch="main")
        >>> df = github.load_ohlcv_for_month(year=2023, month=4, verbose=True)
        >>> print(df.head())
        """
        ...
        for i, file in enumerate(raw_urls, start=1):
            data_df = self.load_ohlcv_csv(file)
    ...
    def load_ohlcv_for_day(self, csv_file, verbose: bool = False) -> pd.DataFrame:
        """
        ...
        Parameters:
        ----------
        csv_file : Name of the specific file to be loaded in the repo.
        ...
        Raises:
        ------
        TODO: Add error management documentation

        Example:
        --------
        >>> github = Github(owner="MapleFrogStudio", repository="DATA-2024-07", branch="main")
        >>> df = github.load_ohlcv_for_day(csv_file='NASDAQ-BM0-2024-07-15.csv', verbose=True)
        >>> print(df.head())
        """
        debug = True
        # Validate input
        # if year < 2023 or month < 1 or month > 12 or day < 1 or day > 31:
        #     raise ValueError(f"Invalid date: {year}-{month:02}-{day:02}. Year must be 2023 or higher, month must be between 1-12, and day must be valid.")        

        # repo_name = f"DATA-{year:4}-{month:02}"
        # day_target = f"{year}-{month:02}-{day:02}"
        # self.repo = repo_name                       # Setup the proper repo to download it's content in json format
        #json_content = self.repo_content()          # Get json of all files contained in repo_name
        #if not isinstance(json_content, list):
        #    raise ValueError(f"Not Found - Unable to load content list from 'repo_content' api method using {repo_name}")         

        json_content = self.repo_content()
        index = next((i for i, d in enumerate(json_content) if d.get('name') == csv_file), None)

        if index is None:
            raise ValueError(f"Not Found - Unable to find {csv_file} in {self.repo}")         
        
        raw_urls = self.select_files(file_list=json_content) 
        df = self.load_ohlcv_csv(raw_urls[index])

        if debug:
            show_json = json_content[0:1]
            print(f'{show_json=}')
            print(f'{index=}')
            print(f'{raw_urls[index]=}')
